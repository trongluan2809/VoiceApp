...
public class Storage extends com.voicerecorder.axet.audiolibrary.app.Storage {
    ...
    public void deleteTmp(File dir) {
        ...
        if (ff == null)
            return;
        for (File f : ff) {
            if (f.isFile() && !f.getAbsolutePath().endsWith("json") && f.lastModified() + StorageProvider.TIMEOUT < now)
                Storage.delete(f);
        }
    }
