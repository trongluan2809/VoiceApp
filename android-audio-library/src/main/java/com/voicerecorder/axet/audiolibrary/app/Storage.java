...
public class Storage extends com.github.axet.androidlibrary.app.Storage {
    ...
    public static boolean openResume = false;
    ...
    public void migrateLocalStorage(File l) {
        ...
        if (!canWrite(l)) return;

//        String extension = FilenameUtils.getExtension(l.getAbsolutePath());
//        Utility.log(l.getAbsolutePath() + " 1 var");
//        String test = l.getAbsolutePath();
        ...
        if (ff == null)
            return;
        for (File f : ff) {
            Utility.log(f.getAbsolutePath() + "---");
            if (f.isFile() && !f.getAbsolutePath().endsWith(".json"))// skip directories (we didn't create one)
                migrate(f, path);
        }
    }

    public Uri migrate(File f, Uri t) {
        // copy folder cũ sang mới
        if(f.getPath().contains(".wav")){
           MainApplication.eVentUpdateData(context);
        }
        Uri u = null;
        try {
            u = com.github.axet.androidlibrary.app.Storage.migrate(context, f, t);
    ...
    public Uri rename(Uri f, String t) {
        Uri u = null;
        try {
            u = com.github.axet.androidlibrary.app.Storage.rename(context, f, t);
        } catch (FileNotFoundException e) {
            e.printStackTrace();
        }
        if (u == null)
